...
int main()
{
    ...
    glw::Buffer buffer_a (GL_ARRAY_BUFFER, GL_STATIC_DRAW, sizeof(write_data), write_data, &error);
    ...
    glw::Buffer buffer_b (GL_ARRAY_BUFFER, GL_STATIC_DRAW, sizeof(write_data), NULL, &error);
    TEST_ASSERT(error == GL_NO_ERROR);
